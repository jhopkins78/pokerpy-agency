...
class CoachAgent(BaseAgent):
    ...
    def generate_coaching_response(self, user_id: str, message: str, context: dict) -> dict:
        ...
        if hasattr(self, "_analyze_question"):
            analysis = self._analyze_question(message, context)
        else:
        ...
        if hasattr(self, "_generate_answer"):
            response = self._generate_answer(user_id, message, context)
        else:
        ...
        # Suggest simulation (legacy, for compatibility)
        ...
        import os
        # If response is a dict with 'suggested_simulation', use that for suggested_simulation
        suggested_simulation = response.get("suggested_simulation") if isinstance(response, dict) and "suggested_simulation" in response else simulation
        return {
            "response": response.get("response") if isinstance(response, dict) and "response" in response else response,
    ...
    def _analyze_question(self, message: str, context: dict) -> dict:
        """
        Inspects the user's message to determine intent.
        If the message includes the word 'simulate', returns {'type': 'simulation_request'}.
        Otherwise, returns {'type': 'chat'}.
        """
        if "simulate" in message.lower():
            return {"type": "simulation_request"}
        return {"type": "chat"}

    def _generate_answer(self, user_id: str, message: str, context: dict) -> dict:
        """
        Generates an answer based on the analysis of the question.
        If the analysis is a simulation request, calls suggest_simulation.
        Otherwise, returns a simple coaching placeholder.
        """
        analysis = self._analyze_question(message, context)
        if analysis.get("type") == "simulation_request":
            result = suggest_simulation(user_id, context)
            return {"suggested_simulation": result}
        else:
            return {"response": "I'm here to help with poker coaching."}
